...
class ScorableField:
    ...
    def __init__(self, assessment_time: float, report_time: float, lon: float, lat: float, alt: float, value):
...
def generate_final_score_and_casualty_assessments(
    ...
    for casualty_id in cats:
        ...
        # Formatting the aggregate must come after scoring because it depends on side effects of scoring
        assessment["aggregate"] = format_aggregate(cats[casualty_id]._aggregate, scored=True)
...
def _format_scorable_field(sf: ScorableField, scored=False):
    """Formats a scorable field.

    Unscored formatting will just return the stored value of the scorable field.
    Scored formatting will include the extra info: the assessment time, the receipt time, the GT used, and
    whether the value was correct or not.

    Args:
        sf (ScorableField): the field to format
        scored (bool): Whether this field has been scored and should be formatted as such

    Returns:
        any: Either the fields value, or a dict containing the value and other metadata
    """
    if not scored:
        return sf.value

    fmt = {}
    fmt["assessment_time"] = sf.assessment_time
    fmt["receipt_time"] = sf.report_time
    fmt["value"] = sf.value
    fmt["correct"] = sf._correct
    if hasattr(sf, "_gt_name"):
        fmt["gt_name"] = sf._gt_name
    return fmt


def format_aggregate(aggregate: dict, scored=False) -> dict:
    ...
    if crk.VITALS in aggregate:
        ...
        if crk.HEART_RATE in aggregate[crk.VITALS]:
            formatted_aggregate[crk.VITALS][crk.HEART_RATE] = _format_scorable_field(
                aggregate[crk.VITALS][crk.HEART_RATE], scored=scored
            )

        if crk.RESPIRATION_RATE in aggregate[crk.VITALS]:
            formatted_aggregate[crk.VITALS][crk.RESPIRATION_RATE] = _format_scorable_field(
                aggregate[crk.VITALS][crk.RESPIRATION_RATE], scored=scored
            )
    ...
    if crk.INJURIES in aggregate:
        ...
        if crk.SEVERE_HEMORRHAGE in aggregate[crk.INJURIES]:
            formatted_aggregate[crk.INJURIES][crk.SEVERE_HEMORRHAGE] = _format_scorable_field(
                aggregate[crk.INJURIES][crk.SEVERE_HEMORRHAGE], scored=scored
            )

        if crk.RESPIRATORY_DISTRESS in aggregate[crk.INJURIES]:
            formatted_aggregate[crk.INJURIES][crk.RESPIRATORY_DISTRESS] = _format_scorable_field(
                aggregate[crk.INJURIES][crk.RESPIRATORY_DISTRESS], scored=scored
            )
        ...
        if crk.TRAUMA in aggregate[crk.INJURIES]:
            ...
            if crk.HEAD in aggregate[crk.INJURIES][crk.TRAUMA]:
                formatted_aggregate[crk.INJURIES][crk.TRAUMA][crk.HEAD] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.TRAUMA][crk.HEAD], scored=scored
                )

            if crk.TORSO in aggregate[crk.INJURIES][crk.TRAUMA]:
                formatted_aggregate[crk.INJURIES][crk.TRAUMA][crk.TORSO] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.TRAUMA][crk.TORSO], scored=scored
                )

            if crk.UPPER_EXTREMITY in aggregate[crk.INJURIES][crk.TRAUMA]:
                formatted_aggregate[crk.INJURIES][crk.TRAUMA][crk.UPPER_EXTREMITY] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.TRAUMA][crk.UPPER_EXTREMITY], scored=scored
                )

            if crk.LOWER_EXTREMITY in aggregate[crk.INJURIES][crk.TRAUMA]:
                formatted_aggregate[crk.INJURIES][crk.TRAUMA][crk.LOWER_EXTREMITY] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.TRAUMA][crk.LOWER_EXTREMITY], scored=scored
                )
        ...
        if crk.ALERTNESS in aggregate[crk.INJURIES]:
            ...
            if crk.OCULAR in aggregate[crk.INJURIES][crk.ALERTNESS]:
                formatted_aggregate[crk.INJURIES][crk.ALERTNESS][crk.OCULAR] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.ALERTNESS][crk.OCULAR], scored=scored
                )

            if crk.VERBAL in aggregate[crk.INJURIES][crk.ALERTNESS]:
                formatted_aggregate[crk.INJURIES][crk.ALERTNESS][crk.VERBAL] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.ALERTNESS][crk.VERBAL], scored=scored
                )

            if crk.MOTOR in aggregate[crk.INJURIES][crk.ALERTNESS]:
                formatted_aggregate[crk.INJURIES][crk.ALERTNESS][crk.MOTOR] = _format_scorable_field(
                    aggregate[crk.INJURIES][crk.ALERTNESS][crk.MOTOR], scored=scored
                )
...
def score_vitals(
    ...
    if crk.VITALS in cat._aggregate:
        ...
        if crk.HEART_RATE in cat._aggregate[crk.VITALS]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if abs(row[gtk.HEART_RATE] - field.value) < hr_tolerance:
                        field._correct = True
                        score += 1
                        vitals_bonus_count += 1 if field.report_time <= golden_window else 0
        ...
        if crk.RESPIRATION_RATE in cat._aggregate[crk.VITALS]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if abs(avg_rr - field.value) < rr_tolerance:
                        field._correct = True
                        score += 1
                        vitals_bonus_count += 1 if field.report_time <= golden_window else 0
        ...
        if vitals_bonus_count == 2:
            # Both vitals were correct and within golden window, so bonus point
            score += 1
        ...
        if crk.SEVERE_HEMORRHAGE in cat._aggregate[crk.INJURIES]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if gt_severe_hemorrhage_as_bool == field.value:
                        field._correct = True
                        score += 4 if field.report_time <= golden_window else 2
        ...
        if crk.RESPIRATORY_DISTRESS in cat._aggregate[crk.INJURIES]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    # Below is the proper implementation w/o the special Phase 1 case of allowing multiple correct answers
                    ...
                    # Below is the implementation that allows for multiple correct answers
                    # This is necessary in Phase 1 due to ambiguous or conflicting definitions for respiratory distress
                    # that make it difficult to discern the correct value on some casualties
                    val = "present" if field.value else "absent"  # Mapping field's bool back into the GT values
                    correct = val in row[gtk.HA_RESPIRATORY_DISTRESS].lower().split("|")
                    if correct:
                        field._correct = True
                        score += 4 if field.report_time <= golden_window else 2
        ...
        if crk.HEAD in cat._aggregate[crk.INJURIES][crk.TRAUMA]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if row[gtk.HA_HEAD_TRAUMA].lower() == field.value.lower():
                        field._correct = True
                        trauma_correct_count += 1
        ...
        if crk.TORSO in cat._aggregate[crk.INJURIES][crk.TRAUMA]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if row[gtk.HA_TORSO_TRAUMA].lower() == field.value.lower():
                        field._correct = True
                        trauma_correct_count += 1
            trauma_gts_used.add(location)
        ...
        if crk.UPPER_EXTREMITY in cat._aggregate[crk.INJURIES][crk.TRAUMA]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if row[gtk.HA_UPPER_EXTREMITY_TRAUMA].lower() == field.value.lower():
                        field._correct = True
                        trauma_correct_count += 1
            trauma_gts_used.add(location)
        ...
        if crk.LOWER_EXTREMITY in cat._aggregate[crk.INJURIES][crk.TRAUMA]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if row[gtk.HA_LOWER_EXTREMITY_TRAUMA].lower() == field.value.lower():
                        field._correct = True
                        trauma_correct_count += 1
            trauma_gts_used.add(location)
        ...
        if crk.OCULAR in cat._aggregate[crk.INJURIES][crk.ALERTNESS]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if row[gtk.HA_OCULAR_ALERTNESS].lower() == field.value.lower():
                        field._correct = True
                        alertness_correct_count += 1
        ...
        if crk.VERBAL in cat._aggregate[crk.INJURIES][crk.ALERTNESS]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    if row[gtk.HA_VERBAL_ALERTNESS].lower() == field.value.lower():
                        field._correct = True
                        alertness_correct_count += 1
            alertness_gts_used.add(location)
        ...
        if crk.MOTOR in cat._aggregate[crk.INJURIES][crk.ALERTNESS]:
            ...
            if location in gts_used:
                ...
                if row is None:
                    ...
                    # Below is the implementation that allows for multiple correct answers
                    # This is necessary in Phase 1 due to ambiguous or conflicting definitions for motor alertness
                    # that make it difficult to discern the correct value on some casualties
                    correct = field.value.lower() in row[gtk.HA_MOTOR_ALERTNESS].lower().split("|")
                    if correct:
                        field._correct = True
                        alertness_correct_count += 1
...
def get_time_proximal_row(
    ...
    while start_idx > 0 and ground_truth.iloc[start_idx][gtk.TIME] > round(sf.assessment_time - time_window, 2):
        start_idx -= 1
