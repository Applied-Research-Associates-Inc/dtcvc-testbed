...
class SimulationGNSSNode(Node):
    ...
    def is_stationary(self, msg):
        ...
        if (
            abs(self._last_imu_message.angular_velocity.x - self._current_imu_message.angular_velocity.x)
            > self._imu_eps
        ):
            self._is_stationary = False
        if (
            abs(self._last_imu_message.angular_velocity.y - self._current_imu_message.angular_velocity.y)
            > self._imu_eps
        ):
            self._is_stationary = False
        if (
            abs(self._last_imu_message.angular_velocity.z - self._current_imu_message.angular_velocity.z)
            > self._imu_eps
        ):
            self._is_stationary = False
...
def _setup_waypoint_actors(world, scenario_file, bp_library):
    ...
    iteration = 1
    last_zone = 0
    for waypoint in scenario_file['waypoints']:
...
def _setup_vehicle_actors(world, scenario_file, bp_library):
    ...
    try:
        ...
        sensor_spawn = carla.Transform(
            location=carla.Location(x=0, y=0, z=1.2), rotation=carla.Rotation(roll=0, pitch=-6.305, yaw=0)
        )
        ...
        sensor.set_attribute("sensor_tick", "0.05")
        sensor_spawn = carla.Transform(
            location=carla.Location(x=0, y=0, z=1.2), rotation=carla.Rotation(roll=0, pitch=-20, yaw=0)
        )
        sensor_actor = world.spawn_actor(sensor, sensor_spawn, attach_to=vehicle)
        sensor_actor.enable_for_ros()
        actors.append(sensor_actor)
        ...
        sensor.set_attribute("sensor_tick", "0.1")
        sensor_spawn = carla.Transform(
            location=carla.Location(x=0, y=0, z=0), rotation=carla.Rotation(roll=0, pitch=0, yaw=0)
        )
        sensor_actor = world.spawn_actor(sensor, sensor_spawn, attach_to=vehicle)
        sensor_actor.enable_for_ros()
        actors.append(sensor_actor)
    ...
    try:
        ...
        while True:
            try:
                ...
                if (
                    simulation_gnss_node.get_current_waypoint() != 0
                    and not simulation_gnss_node.get_current_waypoint() > num_waypoints
                ):
                ...
                if stationary_count >= 5 / settings.fixed_delta_seconds and simulation_gnss_node.get_current_waypoint() >= num_waypoints:
                    ...
                    simulation_status_node.set_status(False)
                    break
